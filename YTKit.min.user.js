...
`;
  ...
  * { scrollbar-width: thin !important; scrollbar-color: rgba(255,255,255,0.2) transparent !important; }`),
  ...
  if (vid) {
  vid.style.setProperty('will-change', 'transform');
  requestAnimationFrame(() => {
  vid.style.removeProperty('will-change');
  });
  }
  ...
  const { btn, countSpan } = makeNavBtn(cat, config, (ICONS.shield || ICONS.settings)(), st ? `${st.blocked}` : '0', 'Ads blocked this session');
  ...
  const { btn } = makeNavBtn(cat, config, (ICONS['eye-off'] || ICONS.settings)(), `${videoCount + channelCount}`, 'Hidden videos + blocked channels');
  sidebar.appendChild(btn);
  return;
  }
