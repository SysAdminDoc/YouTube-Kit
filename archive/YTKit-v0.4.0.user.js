...
// @version      0.4.0
// @description  Ultimate YouTube customization with VLC streaming, local downloads, video hiding, and enhanced viewing experience.
...
(function() {
    ...
    const features = [
        ...
        },
        {
            ...
            name: 'Hide Videos Button',
            description: 'Add X button to hide videos from home/feeds. Hidden videos persist across sessions.',
            group: 'Content',
            icon: 'eye-off',
            _styleElement: null,
            _observer: null,
            ...
            _setHiddenVideos(videos) {
                try {
                    GM_setValue(this._STORAGE_KEY, videos);
                } catch(e) {
                    localStorage.setItem(this._STORAGE_KEY, JSON.stringify(videos));
                }
            },
            ...
            _extractVideoId(element) {
                ...
                if (videoIdEl) return videoIdEl.getAttribute('data-video-id');
                return null;
            },
            ...
            _findThumbnailContainer(element) {
                ...
                for (const sel of selectors) {
                    const container = element.querySelector(sel);
            ...
            _createHideButton() {
                ...
                btn.appendChild(this._createSVG());
                return btn;
            },
            ...
            _showHideToast() {
                ...
                if (!toast) {
                    ...
                    span.textContent = 'Video hidden';
                    toast.appendChild(span);
                    const undoBtn = document.createElement('button');
                    undoBtn.textContent = 'Undo';
                    undoBtn.addEventListener('click', () => this._undoHide());
                    toast.appendChild(undoBtn);
                    const clearBtn = document.createElement('button');
                    clearBtn.textContent = 'Clear All';
                    clearBtn.addEventListener('click', () => {
                        if (confirm('Unhide all ' + this._getHiddenVideos().length + ' hidden videos?')) {
                            this._clearAllHidden();
                        }
                    });
            ...
            _undoHide() {
                ...
                if (this._lastHidden.element) {
                    this._lastHidden.element.classList.remove('ytkit-video-hidden');
                }
                this._lastHidden = null;
                const toast = document.getElementById('ytkit-hide-toast');
                if (toast) toast.classList.remove('show');
            },

            _clearAllHidden() {
                this._setHiddenVideos([]);
                document.querySelectorAll('.ytkit-video-hidden').forEach(el => {
                    el.classList.remove('ytkit-video-hidden');
                });
                const toast = document.getElementById('ytkit-hide-toast');
                if (toast) toast.classList.remove('show');
                showToast('All hidden videos restored', '#3ea6ff');
            },
            ...
            _hideVideo(videoId, element) {
                ...
                this._lastHidden = { id: videoId, element: element };
                this._showHideToast();
            },
            ...
            _processVideoElement(element) {
                ...
                btn.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    this._hideVideo(videoId, element);
                });
            ...
            _processAllVideos() {
                ...
                document.querySelectorAll(selectors).forEach(el => this._processVideoElement(el));
            },
            ...
            init() {
                const css = `
                    ...
                    .ytkit-video-hide-btn:active { transform: scale(0.95); }
                    ...
                    for (const mutation of mutations) {
                        for (const node of mutation.addedNodes) {
                            ...
                            if (node.querySelectorAll) {
                                node.querySelectorAll(videoSelectors).forEach(el => this._processVideoElement(el));
                            }
                        }
                    }
                });
                ...
                console.log('[YTKit] Video Hider initialized. Hidden videos:', this._getHiddenVideos().length);
            },
            ...
            destroy() {
                ...
                document.querySelectorAll('.ytkit-video-hide-btn').forEach(btn => btn.remove());
                document.querySelectorAll('.ytkit-video-hidden').forEach(el => {
                    el.classList.remove('ytkit-video-hidden');
                });
