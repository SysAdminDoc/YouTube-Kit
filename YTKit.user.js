...
// @version      25.9.6
// @description  Ultimate YouTube customization with ad blocking, VLC streaming, video/channel hiding, playback enhancements, theater split, ChapterForge AI chapters, and more.
...
// @exclude      https://*.godlikeproductions.com/*
...
(function ytAdBlockBootstrap() {
    ...
    function pageContextEngine(cfg, W) {
        ...
        // Pre-compiled regex for O(1) URL matching instead of 9x indexOf per request
        const INTERCEPT_RE = /\/youtubei\/v1\/(player|get_watch|browse|search|next|log_event|ad_break)|\/watch\?|\/playlist\?list=|\/reel_watch_sequence/;
        function matchesIntercept(url) {
            if (!url) return false;
            return INTERCEPT_RE.test(url);
        }
        // Pre-compiled regex for single-pass string replacement instead of 4x split/join
        const REPLACE_RE = /"(adPlacements|adSlots|playerAds|adBreakHeartbeatParams)"/g;
        function replaceAdKeys(text) {
            if (typeof text !== 'string') return text;
            return text.replace(REPLACE_RE, '"no_ads"');
        }

        // ── Deep Recursive Ad Pruner (optimized: for..in instead of Object.keys() arrays) ──
        function deepPruneAds(obj, depth) {
            if (!obj || typeof obj !== 'object' || (depth || 0) > 12) return false;
            var pruned = false, d = (depth || 0) + 1, key;
            if (W.Array.isArray(obj)) {
                for (var i = obj.length - 1; i >= 0; i--) {
                    var item = obj[i];
                    if (item && typeof item === 'object') {
                        var isAd = false;
                        for (key in item) { if (AD_RENDERER_SET[key]) { isAd = true; break; } }
                        if (isAd) { obj.splice(i, 1); pruned = true; continue; }
                        var content = item.content || item.renderer;
                        if (content && typeof content === 'object') {
                            for (key in content) { if (AD_RENDERER_SET[key]) { isAd = true; break; } }
                            if (isAd) { obj.splice(i, 1); pruned = true; continue; }
                        }
                        var rir = item.richItemRenderer;
                        if (rir && rir.content && typeof rir.content === 'object') {
                            for (key in rir.content) { if (AD_RENDERER_SET[key]) { isAd = true; break; } }
                            if (isAd) { obj.splice(i, 1); pruned = true; continue; }
                        }
        ...
        safeOverride(W.JSON, 'parse', new W.Proxy(origParse, {
            apply: function(target, thisArg, args) {
                var result = W.Reflect.apply(target, thisArg, args);
                try {
                    // Skip pruning for primitives, null, arrays without objects, and tiny objects
                    if (result && typeof result === 'object' && !W.Array.isArray(result) && pruneObject(result)) stats.blocked++;
                } catch(e) {}
                return result;
            }
        }));
        ...
        safeOverride(W, 'fetch', new W.Proxy(origFetch, {
            apply: function(target, thisArg, args) {
                ...
                } catch(e) { console.debug('[YTKit AdBlock] fetch body prune error:', e); }
                ...
                return W.Reflect.apply(target, thisArg, args).then(function(resp) {
                    ...
                    return resp.clone().text().then(function(text) {
                        try {
                            // Parse once, prune in-place, stringify once (no redundant replaceAdKeys string pass)
                            var obj = origParse(text);
                            if (obj && typeof obj === 'object' && pruneObject(obj)) {
                                stats.blocked++;
        ...
        safeOverride(W.XMLHttpRequest.prototype, 'send', function(body) {
            ...
            const xhr = this;
            if (!xhr._ytab_listener_attached) {
                xhr._ytab_listener_attached = true;
                ...
                try {
                    ...
                    var obj = origParse(text);
                    if (obj && typeof obj === 'object' && pruneObject(obj)) {
                        var newText = W.JSON.stringify(obj);
                ...
                } catch(e) { console.debug('[YTKit AdBlock] XHR intercept error:', e); }
                });
            }
        ...
        safeOverride(W, 'setTimeout', new W.Proxy(origSetTimeout, {
            apply: function(target, thisArg, args) {
                ...
                // Neutralize ad-blocker detection timers (typically 16-18s enforcement checks)
                if (typeof fn === 'function' && delay >= 16000 && delay <= 18000) {
                    try {
                        const src = fn.toString();
                        // Skip native functions; target user-defined detection callbacks
                        if (src.indexOf('[native code]') === -1 && src.length > 30) {
                            args[1] = 1; // Collapse delay to prevent enforcement popups
                        }
                    } catch(e) {}
                }
                return W.Reflect.apply(target, thisArg, args);
        ...
        if (cfg.antiDetect) {
            ...
            // Single regex compiled once (much faster than 6 indexOf calls per .then())
            const AD_DETECT_RE = /abnormal|adsBlocked|adBlock|AdNotAllowed|enforcement|adPlacements/;
            var _noop = function(){};
                ...
                apply: function(target, thisArg, args) {
                    if (typeof args[0] === 'function') {
                        try {
                            ...
                            // Skip native code and short functions (< 50 chars can't contain detection logic)
                            if (src.length > 50 && src.indexOf('[native code]') === -1 && AD_DETECT_RE.test(src)) {
                                args[0] = _noop;
                                stats.blocked++;
                            }
                        } catch(e) {}
        ...
        // Strategy: Multi-layered — skip button click > playback rate acceleration > seek to end
        // Uses MutationObserver on player + low-frequency poll as fallback.
        var adNeutTimer = null;
        var adObserver = null;
        var _adSpeedActive = false;
        var _savedPlaybackRate = 1;
        var _savedVolume = 1;

        function trySkipAd() {
            try {
                // Click skip buttons (comprehensive selector list from multiple scripts)
                var skipSelectors = [
                    '.ytp-ad-skip-button',
                ...
                for (var s = 0; s < skipSelectors.length; s++) {
                    ...
                    for (var b = 0; b < btns.length; b++) {
                        try { btns[b].click(); skipped = true; stats.blocked++; } catch(e) {}
                    }
                }
                ...
                if (player) {
                    if (player.skipAd) try { player.skipAd(); skipped = true; } catch(e) {}
                    if (player.cancelPlayback) try { player.cancelPlayback(); skipped = true; } catch(e) {}
                }

                // Dismiss enforcement/anti-adblock popups
                var popups = W.document.querySelectorAll(
                ...
                for (var pp = 0; pp < popups.length; pp++) {
                    try {
                        if (popups[pp].tagName === 'BUTTON' || popups[pp].id === 'dismiss-button') {
                            popups[pp].click();
                        } else {
                            popups[pp].remove();
                        }
                ...
                // Fallback: If skip button not available, accelerate + mute the ad
                if (!skipped) {
                    var video = W.document.querySelector('.ad-showing video');
                    if (video && !video.ended) {
                        if (!_adSpeedActive) {
                            _savedPlaybackRate = video.playbackRate;
                            _savedVolume = video.volume;
                            _adSpeedActive = true;
                        }
                        video.playbackRate = 16;
                        video.volume = 0;
                        // Also try seeking to end for immediate skip
                ...
                // Remove #player-ads container
                var playerAds = W.document.getElementById('player-ads');
                if (playerAds && playerAds.children.length > 0) {
                    while (playerAds.firstChild) playerAds.removeChild(playerAds.firstChild);
                }
            } catch(e) {}
        }

        function restoreAfterAd() {
            if (!_adSpeedActive) return;
            var video = W.document.querySelector('video');
            if (video) {
                video.playbackRate = _savedPlaybackRate || 1;
                video.volume = _savedVolume || 1;
            }
            _adSpeedActive = false;
        }
                    ...
                    for (var i = 0; i < mutations.length; i++) {
                        if (mutations[i].attributeName === 'class') {
                            ...
                            if (el.classList.contains('ad-showing')) {
                                stats.ssapSkipped++;
                                // Immediate skip attempt + retries with jitter
                                trySkipAd();
                                W.setTimeout(trySkipAd, 300 + W.Math.random() * 200);
                                W.setTimeout(trySkipAd, 1000 + W.Math.random() * 500);
                                W.setTimeout(trySkipAd, 2500 + W.Math.random() * 500);
                                W.setTimeout(trySkipAd, 5000 + W.Math.random() * 1000);
                            } else {
                                // Ad ended — restore normal playback state
                                restoreAfterAd();
                            }
                        }
                    }
    ...
    const _debouncedEnsureCSS = () => {
        if (_cssDebounceTmr) return;
        _cssDebounceTmr = setTimeout(() => { _cssDebounceTmr = null; _ensureCSS(); }, 100);
    };
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _ensureCSS);
    const _cssObserver = new MutationObserver(_debouncedEnsureCSS);
    ...
    // Pre-built selector string: single querySelectorAll instead of 22+ separate calls
    const _adScanSelector = [...AD_REMOVAL_TAGS].map(t => t.toLowerCase()).join(',') + ',[layout*="display-ad-"]';
    function scanForAds(root) {
        if (!root || typeof root.querySelectorAll !== 'function') return;
        for (const el of root.querySelectorAll(_adScanSelector)) nukeAdNode(el);
    }
    let _domCleanerObserver = null;
    function startDOMCleaner() {
        if (_domCleanerObserver) return;
        scanForAds(document);
        _domCleanerObserver = new MutationObserver(mutations => {
        ...
        _domCleanerObserver.observe(document.documentElement, { childList: true, subtree: true });
    }
    ...
    try { _patchAPI(); } catch(e) { console.debug('[YTKit] API patch failed:', e); }
    setTimeout(() => { try { _patchAPI(); } catch(e) { console.debug('[YTKit] API patch retry failed:', e); } }, 0);
})();
...
(function() {
    ...
    // Bridge to page document — Tampermonkey MV3 may run scripts inside a sandboxed
    // userscript.html iframe, so bare `document` points to the iframe's DOM instead of
    // YouTube's. Shadowing it here ensures every querySelector, appendChild, createElement
    // etc. in this entire IIFE targets the real page.
    const document = _rw.document;
    ...
    const TranscriptService = {
        ...
        _getInnertubeApiKey() {
            // Try ytcfg first (cheap), fall back to script tag search (avoids full body innerHTML)
            try { const k = window.ytcfg?.get?.('INNERTUBE_API_KEY'); if (k) return k; } catch(e) {}
    ...
    // ── Global Video ID helper ──
    let _cachedHref = '', _cachedVid = null;
    function getVideoId() {
    ...
    const VideoTypeDetector = {
        ...
        _fromPlayerResponse() {
            try {
                ...
                if (d.videoId && d.videoId !== vid) return null;
    ...
    function startButtonChecker() {
        ...
        if (!buttonObserver) {
            ...
            // Scope to watch-flexy if available (95% of use), fall back to body
            const watchFlexy = document.querySelector('ytd-watch-flexy');
            buttonObserver.observe(watchFlexy || document.body, { childList: true, subtree: true });
        }
        ...
        addNavigateRule('_buttonChecker', () => {
            ...
            lastVideoId = null;
            checkAllButtons();
            spaTimer = setTimeout(checkAllButtons, 1000);
        });
    }
    ...
    const cleanupNavigateListeners = () => {
        if (!isNavigateListenerAttached) return;
        document.removeEventListener('yt-navigate-finish', debouncedRunNavigateRules);
        _rw.removeEventListener('popstate', debouncedRunNavigateRules);
        if (_watchFlexyObserver) { _watchFlexyObserver.disconnect(); _watchFlexyObserver = null; }
    ...
    const ensureNavigateListener = () => {
        ...
        _rw.addEventListener('popstate', debouncedRunNavigateRules);

        // Fallback: History API monkey-patching for pushState/replaceState navigation
        if (!_rw._ytkit_history_patched) {
            const origPush = _rw.history.pushState.bind(_rw.history);
            const origReplace = _rw.history.replaceState.bind(_rw.history);
            _rw.history.pushState = function() {
                const ret = origPush.apply(this, arguments);
                debouncedRunNavigateRules();
                return ret;
            };
            _rw.history.replaceState = function() {
                const ret = origReplace.apply(this, arguments);
                debouncedRunNavigateRules();
                return ret;
            };
            _rw._ytkit_history_patched = true;
        }
    ...
    const settingsManager = {
        ...
        _migrateOldSettings(saved) {
            ...
            delete migrated.deArrow;
            delete migrated.daReplaceTitles;
            delete migrated.daReplaceThumbs;
            delete migrated.daTitleFormat;
            delete migrated.daFallbackFormat;
            delete migrated.daShowOriginalHover;
            delete migrated.daCacheTTL;
            delete migrated.daDebugLog;
        ...
        importAllSettings(jsonString) {
            try {
                ...
                // Validate imported settings against defaults schema
                const newSettings = { ...this.defaults };
                for (const [key, value] of Object.entries(settings)) {
                    if (key in this.defaults) {
                        const expected = typeof this.defaults[key];
                        if (typeof value === expected || (Array.isArray(this.defaults[key]) && Array.isArray(value))) {
                            newSettings[key] = value;
                        } else {
                            console.warn('[YTKit] Skipping invalid import setting:', key, '(expected', expected, 'got', typeof value + ')');
                        }
                    }
                }
    ...
    const features = [
        ...
        },
        {
            ...
            init() {
                ...
                this._styleEl.id = 'ytkit-player-overlay-style';
            ...
            'ytd-watch-metadata #bottom-row'),
        {
            ...
            _dprQuery: null,
            _dprHandler: null,
            _monitorResizeHandler: null,
            _remountDebounce: null,           // which element receives scroll/wheel handlers
            ...
            _triggerPlayerResize() {
                ...
                this._resizeTimer = setTimeout(() => {
                    ...
                    // Skip window resize dispatch for live/VOD — YouTube's handler
                    // recalculates dimensions on the reparented player, which causes
                    // Chrome to re-composite the video layer and kill the MSE stream
            ...
            _mountOverlay() {
                ...
                let _fpsCount = 0;
                const _isLiveType = this._videoType === 'live' || this._videoType === 'vod';
                const forcePlayerSize = () => {
                    ...
                    requestAnimationFrame(() => {
                        ...
                        // NEVER touch the <video> element directly for live/VOD —
                        // setting inline dimensions causes Chrome to re-composite the
                        // video layer, which detaches the MSE decoder output surface
                        // and kills the live stream. CSS rules handle sizing instead.
                        if (!_isLiveType) {
                ...
                // For live: only run once (initial sizing), don't keep resetting the counter
                let _resizeDebounce = null;
                let _roFired = 0;
                this._playerResizeObs = new ResizeObserver(() => {
                    clearTimeout(_resizeDebounce);
                    _roFired++;
                    // Live: stop after 2 cycles to prevent dimension-setting from killing MSE
                    if (_isLiveType && _roFired > 2) return;
                ...
                // Skip for live/VOD: the resize event causes YT's player to recalculate
                // dimensions on the reparented player, which breaks the MSE live connection
                if (!_isLiveType) {
                ...
                // Live/VOD: reparenting breaks MSE source buffer connection.
                // Schedule aggressive recovery to re-establish live edge.
                if (this._videoType === 'live' || this._videoType === 'vod') {
                    this._ensureLiveVideoRendering();
                }
            },

            // Verify live video is rendering after reparent; recover if black
            _ensureLiveVideoRendering(attempt = 0) {
                if (!this._isActive || attempt > 4) return;
                ...
                const check = () => {
                    ...
                    const isBlack = vid.paused || decoded === 0 ||
                        (vid.videoWidth === 0 && vid.videoHeight === 0);

                    if (!isBlack) {
                        DebugManager.log('Theater', `Live video OK (${decoded} frames decoded)`);
                        return;
                    }

                    DebugManager.log('Theater', `Live recovery attempt ${attempt + 1}`);

                    if (attempt === 0) {
                        // Try 1: seekToLiveHead via YT player API
                        try {
                            if (typeof mp.seekToLiveHead === 'function') {
                                mp.seekToLiveHead();
                                DebugManager.log('Theater', 'Live: seekToLiveHead()');
                            } else if (typeof mp.seekToStreamTime === 'function') {
                                mp.seekToStreamTime();
                            }
                        } catch (e) {}
                        vid.play().catch(() => {});
                    } else if (attempt === 1) {
                        // Try 2: click the LIVE badge
                        const liveBadge = mp.querySelector('.ytp-live-badge');
                        if (liveBadge) {
                            liveBadge.click();
                            DebugManager.log('Theater', 'Live: clicked live badge');
                        }
                    } else if (attempt === 2) {
                        // Try 3: seek to end of seekable range (live edge)
                        if (vid.seekable && vid.seekable.length > 0) {
                            vid.currentTime = vid.seekable.end(vid.seekable.length - 1);
                            vid.play().catch(() => {});
                            DebugManager.log('Theater', 'Live: seeked to live edge');
                        }
                    } else if (attempt === 3) {
                        // Try 4: nuclear — reload video via YT API
                        const videoId = getVideoId();
                        try {
                            if (typeof mp.loadVideoById === 'function' && videoId) {
                                mp.loadVideoById(videoId);
                                DebugManager.log('Theater', 'Live: reloaded video');
                            } else if (typeof mp.loadVideoByPlayerVars === 'function') {
                                mp.loadVideoByPlayerVars({ video_id: videoId });
                            }
                        } catch (e) {
                            DebugManager.log('Theater', 'Live reload failed:', e);
                        }
                    }

                    // Schedule next attempt with increasing delay
                    setTimeout(() => this._ensureLiveVideoRendering(attempt + 1),
                        800 + attempt * 500);
                };

                // First check: give the browser time to settle after reparent
                setTimeout(check, attempt === 0 ? 1200 : 200);
            },
            ...
            // ── Monitor change: full unmount/remount cycle ──
            _handleMonitorChange() {
                if (!this._isActive || !window.location.pathname.startsWith('/watch')) return;
                clearTimeout(this._remountDebounce);
                this._remountDebounce = setTimeout(() => {
                    if (!this._isActive) return;
                    const wasSplit = this._isSplit;
                    const wasLive = this._videoType === 'live' || this._videoType === 'vod';
                    DebugManager.log('Theater', 'Monitor change — remounting');
                    this._unmount(true);
                    requestAnimationFrame(() => {
                        requestAnimationFrame(() => {
                            if (!window.location.pathname.startsWith('/watch')) return;
                            ...
                            if (wasSplit) {
                                setTimeout(() => {
                                    if (this._isActive && !this._isSplit) this._expandSplit();
                                }, 400);
                            }
                            // Live streams: MSE source buffer detaches during
                            // reparent. Use shared live recovery.
                            if (wasLive) {
                                this._ensureLiveVideoRendering();
                            }
                        });
            ...
            _startMonitorDetection() {
                const setupDpr = () => {
                    ...
                    this._dprHandler = () => { this._handleMonitorChange(); setupDpr(); };
                    this._dprQuery.addEventListener('change', this._dprHandler);
                };
                setupDpr();
                let _lsx = window.screenX, _lsy = window.screenY;
                this._monitorResizeHandler = () => {
                    const dx = Math.abs(window.screenX - _lsx), dy = Math.abs(window.screenY - _lsy);
                    _lsx = window.screenX; _lsy = window.screenY;
                    if (dx > 150 || dy > 150) this._handleMonitorChange();
                };
                window.addEventListener('resize', this._monitorResizeHandler);
            },

            _stopMonitorDetection() {
                if (this._dprQuery) this._dprQuery.removeEventListener('change', this._dprHandler);
                this._dprQuery = null; this._dprHandler = null;
                if (this._monitorResizeHandler) window.removeEventListener('resize', this._monitorResizeHandler);
                this._monitorResizeHandler = null;
                clearTimeout(this._remountDebounce);
            },
        ...
        // ─── Playback Speed OSD ───
        {
            ...
            init() {
                const self = this;
                this._rateHandler = function() {
                    const speed = this.playbackRate;
                    if (self._lastSpeed !== null && speed !== self._lastSpeed) {
                        self._showOSD(speed);
                    }
                    self._lastSpeed = speed;
                };
                // Use ratechange event instead of 200ms polling
                const attachToVideo = () => {
                    const video = document.querySelector('video.html5-main-video');
                    if (video && video !== self._videoEl) {
                        if (self._videoEl) self._videoEl.removeEventListener('ratechange', self._rateHandler);
                        self._videoEl = video;
                        self._lastSpeed = video.playbackRate;
                        video.addEventListener('ratechange', self._rateHandler);
                    }
                };
                attachToVideo();
                // Re-attach on SPA navigation (new video element)
                this._navHandler = () => setTimeout(attachToVideo, 500);
                document.addEventListener('yt-navigate-finish', this._navHandler);
            },
            _showOSD(speed) {
                const player = document.querySelector('#movie_player');
            ...
            destroy() {
                if (this._videoEl) this._videoEl.removeEventListener('ratechange', this._rateHandler);
...
- Output ONLY the outline. Nothing before. Nothing after.`;
            ...
            async _fetchTranscript(videoId, onStatus) {
                ...
                try {
                    ...
                    if (!apiKey) { this._log('M5: no API key found, skipping'); throw new Error('No Innertube API key'); }
                    let clientVersion; try { clientVersion = pw.ytcfg?.get?.('INNERTUBE_CLIENT_VERSION'); } catch(e) { console.debug('[YTKit] ytcfg access failed:', e); }
            ...
            async _fetchTranscriptViaInnertube(videoId, lang) {
                ...
                let apiKey; try { apiKey = pw.ytcfg?.get?.('INNERTUBE_API_KEY'); } catch(e) {}
                if (!apiKey) { this._log('get_transcript: no API key found'); return null; }
                let clientVersion; try { clientVersion = pw.ytcfg?.get?.('INNERTUBE_CLIENT_VERSION'); } catch(e) { console.debug('[YTKit] ytcfg access failed:', e); }
                if (!clientVersion) clientVersion = '2.20250210.01.00';
            ...
            async _getAudioStreamUrl(videoId) {
                ...
                if (!sources.some(f => f.mimeType?.startsWith('audio/') && f.url)) {
                    ...
                    let apiKey; try { apiKey = pw.ytcfg?.get?.('INNERTUBE_API_KEY'); } catch(e) { console.debug('[YTKit] ytcfg access failed:', e); }
                    if (!apiKey) { this._log('Audio: no API key found, skipping Innertube fallback'); }
...
Rules: start in seconds (integers), first at 0, 3-8 chapters, 2-6 POIs. Titles 3-7 words, specific to content discussed. Output ONLY JSON.`;
            ...
            _renderProgressBarOverlay() {
                ...
                if (this._lastTranscriptSegments?.length) {
                    ...
                    overlay.addEventListener('mousemove', (e) => {
                        if (_tipRafPending) return;
                        _tipRafPending = true;
                        requestAnimationFrame(() => { _tipRafPending = false;
            ...
            _renderPanel() {
                ...
                TrustedHTML.setHTML(this._panelEl, `
                    <div class="cf-panel-header"><div><span class="cf-panel-title">ChapterForge</span><span class="cf-panel-version">v25.9</span></div><button class="cf-panel-close" id="cf-close">&times;</button></div>
            ...
            init() {
                ...
                if (appState.settings?.cfDebugLog) console.log('[ChapterForge] v25.9 initialized — Provider:', appState.settings.cfLlmProvider || 'builtin');
            },
    ...
    function buildSettingsPanel() {
        ...
        // Group labels: maps first category of each group to label text
        ...
        categoryOrder.forEach((cat, index) => {
            ...
            if (cat === 'Ad Blocker') {
                ...
                // Live update (store ref for cleanup)
                if (window._ytkitAdCountInterval) clearInterval(window._ytkitAdCountInterval);
                window._ytkitAdCountInterval = setInterval(() => {
            ...
            const catId = cat.replace(/ /g, '-');
            const enabledCount = categoryFeatures.filter(f => !f.isSubFeature && ((f.type === 'select' && f.settingKey && !(f.id in appState.settings)) || appState.settings[f.id])).length;
        ...
        function buildAdBlockPane(config) {
            ...
            statsObserver.observe(pane, { attributes: true, attributeFilter: ['class'] });
            // Store ref for cleanup
            pane._ytkitStatsObserver = statsObserver;
            ...
            updateBtn.onclick = () => {
                ...
                try { const u = new URL(url); if (!u.protocol.startsWith('https')) throw 0; } catch { createToast('Invalid filter URL (HTTPS required)', 'error'); updateBtn.textContent = 'Update Filters'; updateBtn.style.opacity = '1'; return; }
...
`;
    ...
    function buildFeatureCard(f, accentColor, isSubFeature = false) {
        ...
        const _cardIsEnabled = f._arrayKey
            ? !!(appState.settings[f._arrayKey] || []).includes?.(f.id)
            : !!appState.settings[f.id];
        card.className = 'ytkit-feature-card' + (_cardIsEnabled && !isSubFeature ? ' ytkit-card-enabled' : '') + (isSubFeature ? ' ytkit-sub-card' : '') + (f.type === 'textarea' ? ' ytkit-textarea-card' : '') + (f.type === 'select' ? ' ytkit-select-card' : '') + (f.type === 'info' ? ' ytkit-info-card' : '');
    ...
    function updateAllToggleStates() {
        ...
        // Update card enabled state indicator
        document.querySelectorAll('.ytkit-feature-card:not(.ytkit-sub-card)').forEach(card => {
            const cb = card.querySelector('.ytkit-feature-cb');
            if (cb) card.classList.toggle('ytkit-card-enabled', cb.checked);
        });
    }
    ...
    function injectPanelStyles() {
        // Direct injection instead of GM_addStyle to ensure styles go into the
        // page's document (not Tampermonkey's sandbox iframe)
        const _panelStyle = document.createElement('style');
        _panelStyle.id = 'ytkit-panel-styles';
...
#ytkit-settings-panel, .ytkit-toast-container {
...
.ytkit-feature-card.ytkit-card-enabled {
    border-left-color: var(--cat-color, var(--ytkit-accent));
}
.ytkit-sub-card {
    margin-left: 24px;
    background: var(--ytkit-bg-elevated);
...
ytd-live-chat-frame:not([style*="position"]) {
...
@keyframes slideDown {
    ...
    to { opacity: 1; transform: translateY(0); }
        ...
        `;
        (document.head || document.documentElement).appendChild(_panelStyle);
    }
    ...
    const PAGE_MODAL_CONFIG = {
        home: [
            { id: 'fiveVideosPerRow',           label: 'Videos Per Row' },
    ...
    function main() {
        ...
        window.ytkit = {
            ...
            list() {
                ...
                features.forEach(f => {
                    ...
                    const isOn = (f.type === 'select' && f.settingKey && !(f.id in appState.settings)) || appState.settings[f.id];
                    (isOn ? enabled : disabled).push(f.id);
                });
        ...
        if (isSafeMode) {
            ...
            features.forEach(f => {
                if (f._arrayKey) return;
                // Select-type features with settingKey are always active (they have a value, not on/off)
                const isEnabled = (f.type === 'select' && f.settingKey && !(f.id in appState.settings))
                    ? true
                    : appState.settings[f.id];
        ...
        console.log(`%c[YTKit] v25.9.4 Initialized${isSafeMode ? ' (SAFE MODE)' : ''}`, 'color: #3b82f6; font-weight: bold; font-size: 14px;');
    }

    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        main();
    } else {
        _rw.addEventListener('DOMContentLoaded', main);
    }
})();