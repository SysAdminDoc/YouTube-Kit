...
### v3.10.0 — Watch Analytics, Subtitle Styling, AI Summary, Chapters
- `watchHistoryAnalytics`: modal with 30-day CSS bar chart of `watchTimeTracker` data. Stats row + per-day bars. Entry: "📊 Watch Stats" button on watch page + `window.__ytkitOpenAnalytics()`.
- `subtitleStyling`: CSS overrides on `.ytp-caption-segment` and `.ytp-caption-window-container`. Settings: `subStyleFontSize`, `subStyleFontFamily`, `subStyleColor`, `subStyleBgColor`, `subStyleBgOpacity`, `subStyleBottomOffset`, `subStyleTextShadow`.
- `aiVideoSummary`: player-button BYO-key summarization. Providers: openai/anthropic/gemini/ollama. Direct `fetch()` from content script (not EXT_FETCH) so arbitrary endpoints work without allowlist changes. Settings: `aiSummaryProvider`, `aiSummaryEndpoint`, `aiSummaryModel`, `aiSummaryApiKey`. Transcript truncated to 120k chars.
- `copyChapterMarkdown`: copies `- [MM:SS](youtu.be/{id}?t=secs) — title` list. Parses `ytd-macro-markers-list-item-renderer` first, falls back to description regex.
- `chapterJumpButtons`: prev/next chapter seek in player right-controls.
