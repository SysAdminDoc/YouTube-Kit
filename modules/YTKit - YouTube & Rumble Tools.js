// ==UserScript==
// @name         YTKit - YouTube & Rumble Tools
// @description  All-in-one toolkit for YouTube: local MP4 & MP3 downloads, restore dislikes, and many other features. (Rumble support is experimental).
...
// @compatible   edge
// @license      MIT
// @downloadURL  https://raw.githubusercontent.com/SysAdminDoc/YTKit/main/modules/YTKit.user.js
// @updateURL    https://raw.githubusercontent.com/SysAdminDoc/YTKit/main/modules/YTKit.user.js
// ==/UserScript==

(function () {
  'use strict';

  // Only run the script's core functionality on YouTube.
  // Rumble support would require a separate implementation due to different site structure.
  if (!window.location.hostname.includes('youtube.com')) {
    console.log('YTKit loaded on a non-YouTube page. Core features disabled.');
    return;
  }
  ...
  const urlSharedCode = "https://github.com/SysAdminDoc/YTKit";
  ...
  const API_KEY = 'dfcb6d76f2f6a9894gjkege8a4ab232222'; // API KEY FOR DOWNLOAD AUDIO VIDEO
  ...
  async function startDownloadVideoOrAudio(format, container) {
    ...
    try {
      const response = await fetch(`${API_URL_AUDIO_VIDEO}format=${format}&url=${encodeURIComponent(videoURL)}&api=${API_KEY}`);
      ...
      const checkProgress = setInterval(async () => {
        try {
          ...
          if (progressData.progress >= 1000 && progressData.download_url) {
            ...
            progressRetryBtn.style.display = 'none';
            container.dataset.downloading = 'false';
            window.open(progressData.download_url);
          }
        } catch (e) {
 ...
 <svg width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-align-box-right-stretch"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M15 17h2" /><path d="M3 5a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v14a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-14z" /><path d="M11 12h6" /><path d="M13 7h4" /></svg>
    ...
    <select class="selectcalidades ocultarframe" required>
      ...
      <div id="descargando" class="download-container ocultarframe">
        ...
        <div class="download-footer">
          <a href="https://github.com/SysAdminDoc/YTKit" target="_blank"> <i class="fa-brands fa-github"></i> by: Matthew Parker</a>
        </div>
      </div>
    </div>
    </form>
   ...
   <div class="container-mdcm">
    <div class="header-mdcm">
      ...
      <div class="icons-mdcm">
        <a href="https://raw.githubusercontent.com/SysAdminDoc/YTKit/main/modules/YTKit.user.js"
          target="_blank">
          <button class="icon-btn-mdcm">
            <i class="fa-solid fa-arrows-rotate"></i>
          </button>
        </a>
        <a href="https://github.com/SysAdminDoc/YTKit" target="_blank">
          <button class="icon-btn-mdcm">
  ...
  <div class="actions-mdcm">
    <div class="developer-mdcm">
      Developed by <a href="https://github.com/SysAdminDoc/YTKit" target="_blank"> <i class="fa-brands fa-github"></i> Matthew Parker</a>
    </div>
    <span style="color: #fff" ;>v3.0.0</span>
  </div>
  `;
  ...
  function saveSettings() {
    ...
    GM_setValue('ytSettingsYTKit', JSON.stringify(settings));
  }
  ...
  function loadSettings() {
    const settings = JSON.parse(GM_getValue('ytSettingsYTKit', '{}'));

    if (settings.theme) {
      $e(`input[name="theme"][value="${settings.theme}"]`).checked = true;
    }
    settings.menu_ytkit = settings.menu_ytkit || {
      bg: "#252525",
      color: "#ffffff",
      accent: "#ff0000"
    };
    ...
    selectedBgColor = settings.menu_ytkit.bg;
    selectedTextColor = settings.menu_ytkit.color;
    selectedBgAccentColor = settings.menu_ytkit.accent;
  ...
  function applySettings() {
    ...
    $sp('--yt-enhance-menu-bg', settings.menu_ytkit.bg);
    $sp('--yt-enhance-menu-text', settings.menu_ytkit.color);
    $sp('--yt-enhance-menu-accent', settings.menu_ytkit.accent);
    ...
    if (isWatchPage()) {
      setTimeout(() => {
        ...
        // If sync is active, it works as before
        if (settings.syncCinematic) {
          ...
          // If NOT active, just show/hide #cinematics
          const cinematicDiv = $id('cinematics');
          if (cinematicDiv) {
    ...
    function checkForVideo() {
      ...
      if ((video && document.location.href.includes('watch')) || miniPlayer) {
        ...
        // Only if the video changed or is not set up
        if (video !== currentVideo || !isSetup) {
          cleanup(true); // Complete cleanup before creating a new one
    ...
    function downloadDescriptionVideo() {
      ...
      if (!containerDescription) {
        console.warn('Description container not found. Button will not be inserted.');
        return;
      }
        ...
        for (let script of ldJson) {
          try {
            ...
            if (data['@type'] === 'VideoObject') {
              const description =
                `📅 Date published: ${data.uploadDate || 'Not available'}\n` +
                `Author: ${data.author || 'Not available'}\n` +
                `🎬 Name video: ${data.name || 'Not available'}\n` +
                `🖼️ Thumbnail: ${Array.isArray(data.thumbnailUrl) ? data.thumbnailUrl.join(', ') : data.thumbnailUrl || 'Not available'}\n` +
                `📝 Description: ${data.description || 'Not available'}\n\n\n` +
                `🎭 Category: ${data.genre || 'Not available'}\n`;
  ...
  function renderizarButtons() {
        ...
        bufferVideo.addEventListener("click", () => {
          ...
          setTimeout(() => {
            ...
            if (option) {
              ...
              console.log("Option not found, try increasing the wait time.");
            }
          }, 1000);
        });
    ...
    // validate dark mode and video window
    // Repeat video button
          ...
          fetch(imageUrl)
            ...
            .then((blob) => {
              ...
              if (imageSizeKB >= 20) {
                ...
                console.log(
                  'The image does not exceed 20 KB. It will not be downloaded.'
                );
              }
            })
            .catch((error) => {
        ...
        if ('pictureInPictureEnabled' in document) {
          if (!document.pictureInPictureElement) {
            ...
            video
              ...
              .catch((error) => {
                console.error(
                  'Error activating Picture-in-Picture mode:',
                  error
                );
              });
          } else {
    ...
    if (checked_updates != undefined) {
      checked_updates.onclick = () => {
        window.open(
          `https://raw.githubusercontent.com/SysAdminDoc/YTKit/main/modules/YTKit.user.js`
        );
      };
    }
  ...
  console.log('YTKit script running...');
  const HEADER_STYLE = 'color: #3399FF; font-size: 24px; font-family: sans-serif;';
  ...
  console.log(
    '%cYTKit - YouTube & Rumble Tools\n' +
    '%cVersion %c(v3.0.0)\n' +
    'By: Matthew Parker (SysAdminDoc).',
    HEADER_STYLE,
    CODE_STYLE,
    MESSAGE_STYLE
  );

  if (!localStorage.getItem('notification-ytkit')) {
    Notify('info', 'YTKit Loaded! Settings available in the top-right gear icon.');
    localStorage.setItem('notification-ytkit', true);
  }
  ...
  $id('export-config').addEventListener('click', () => {
    const settings = GM_getValue('ytSettingsYTKit', '{}');
    $id('config-data').value = settings;
    try {
      JSON.parse(settings); // Validate JSON
      navigator.clipboard.writeText(settings);
      Notify('success', 'Configuration copied to clipboard!');
    } catch (e) {
      Notify('error', 'Could not export configuration.');
    }
  });
  ...
  $id('import-config').addEventListener('click', () => {
    ...
    if (!configData) {
        Notify('warning', 'Please paste configuration data first.');
        return;
    }
    try {
      JSON.parse(configData); // Validate JSON
      GM_setValue('ytSettingsYTKit', configData);
      setTimeout(() => {
        Notify('success', 'Configuration imported successfully! Reloading...');
  ...
  checkActiveWave.addEventListener('change', () => {
    ...
    if (waveVisualizer.checked) {
      ...
      hideCanvas();
      Notify('success', 'Wave visualizer disabled. Please reload the page.');
      setTimeout(() => {
        window.location.reload();
      }, 1000);
    }
  ...
  if (checkCinematicLighting) {
    checkCinematicLighting.addEventListener('change', () => {
      ...
      // Apply behavior according to sync state
      if (syncToggle.checked) {
        // If there is sync, use toggleCinematicLighting
        setTimeout(() => {
          toggleCinematicLighting();
        }, 300);
      } else {
        // If there is no sync, apply display immediately
        if (cinematicDiv) {
          cinematicDiv.style.display = cinematicToggle.checked ? 'block' : 'none';
        }
      }
  ...
  if (checkSyncCinematic) {
    checkSyncCinematic.addEventListener('change', () => {
      ...
      if (syncToggle.checked) {
        ...
        // If sync is enabled and cinematic mode is enabled, sync with YouTube
        if (cinematicToggle.checked) {
          setTimeout(() => {
            toggleCinematicLighting();
          }, 500);
        ...
        // If sync is disabled, immediately apply the toggle state
        if (cinematicDiv) {
          cinematicDiv.style.display = cinematicToggle.checked ? 'block' : 'none';
        }
      }
  ...
  GM_registerMenuCommand('Update YTKit Script', function() {
    window.open('https://raw.githubusercontent.com/SysAdminDoc/YTKit/main/modules/YTKit.user.js', '_blank');
  });
  ...
  // show preview
  const storedImg = localStorage.getItem('backgroundImage');
  if (storedImg) {
    preview.style.backgroundImage = `url(${storedImg})`;
